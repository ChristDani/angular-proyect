  ...
  getAccounts(): void {
    this.accountService.getAccounts().subscribe({
      next: (data) => {
        ...
        this.accountAhorroCount = this.accounts.filter(
          (account) => account.type === 'ahorro'
        ).length;
        this.accountCorrienteCount = this.accounts.filter(
          (account) => account.type === 'corriente'
        ).length;
        const accounts = {
          title: 'Cuentas',
          items: [
  ...
  getLoans(): void {
    this.loanService.getLoans().subscribe({
      next: (data) => {
        ...
        const loans = {
          title: 'Préstamos',
          items: [{ value: this.loanCount, label: 'Cantidad total de Préstamos:' }],
        };
